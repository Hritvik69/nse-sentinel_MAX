...
def _paths_from_input(df: pd.DataFrame | None) -> list[Path]:
    if df is None or not isinstance(df, pd.DataFrame) or df.empty:
        ...
        if app_universe:
            if len(app_universe) >= 2000:
                return [_DATA_DIR / f"{bare}.NS.csv" for bare in app_universe]
            paths: list[Path] = [_DATA_DIR / f"{bare}.NS.csv" for bare in app_universe]
            seen = {path.stem.upper() for path in paths}
    ...
    if "Symbol" in df.columns:
        symbols = [str(v).strip() for v in df["Symbol"].dropna().tolist()]
    elif "Ticker" in df.columns:
        symbols = [str(v).strip() for v in df["Ticker"].dropna().tolist()]

    paths: list[Path] = []
    seen: set[str] = set()
    for raw_symbol in symbols:
        bare = raw_symbol.replace(".NS", "").upper()
        if not bare or bare in seen:
            continue
        seen.add(bare)
        path = _DATA_DIR / f"{bare}.NS.csv"
        if path.exists():
            paths.append(path)
    return paths
