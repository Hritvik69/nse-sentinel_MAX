...
def _clear_all_bt_caches() -> None:
    """
    Clear every mode engine's _BT_CACHE dict.
    ...
    This helper iterates sys.modules and clears _BT_CACHE in any module
    whose name matches a known engine pattern. Called inside both
    activate() and restore() so both directions are clean.
    ...
    _CACHE_MODULE_PATTERNS = (
        ...
        "app",  # app.py also has a module-level _BT_CACHE
    )
    try:
        for mod_name, mod in list(sys.modules.items()):
            if mod is None:
                continue
            # Only target known engine modules, not arbitrary modules
